...
const handleGuess = async (guess: boolean) => {
	if (!currentPokemon.value || store.isLoading) return;

	const correctGuess = currentPokemon.value.sprite.isShiny === guess;
	if (correctGuess) {
